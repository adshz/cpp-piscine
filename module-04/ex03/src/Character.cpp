...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/24 11:01:35 by szhong            #+#    #+#             */
/*   Updated: 2025/07/24 11:01:37 by szhong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Character::~Character(void) {
  std::cout << CYAN "Character destructor called for \"" << this->_name
            << "\"." RESET << std::endl;
...
Character &Character::operator=(Character const &src) {
  std::cout << CYAN "Character assignment operator overload called." RESET
            << std::endl;
...
void Character::equip(AMateria *materia) {
  if (!materia) {
    std::cout << RED << this->_name
              << ": Can't equip something that doesn't exist!" RESET
              << std::endl;
    return;
  }
  for (int i = 0; i < this->INVENTORY_SIZE; i++) {
    if (this->_inventory[i] == NULL) {
      ...
      std::cout << GREEN << this->_name << " equipped " << materia->getType()
                << " materia to inventory index " << i << "." RESET
                << std::endl;
      return;
    }
  }
  std::cout << RED << this->_name << ": I can't carry any more materia!" RESET
            << std::endl;
  return;
}
...
void Character::unequip(int index) {
  ...
  if (this->_inventory[index] == NULL) {
    std::cout << RED << this->_name << ": I don't have anything at index "
              << index << " to unequip!" RESET << std::endl;
    return;
  }
  std::cout << GREEN << this->_name << " unequipped "
            << this->_inventory[index]->getType()
            << " materia from inventory index " << index << "." RESET
            << std::endl;
...
void Character::use(int index, ICharacter &target) {
  if (index < 0 || index >= this->INVENTORY_SIZE) {
    std::cout << this->_name << ": " RED "Can't use item at inventory index "
              << index << ". It's not a valid index!" RESET << std::endl;
    return;
  }
  if (this->_inventory[index] != NULL) {
    std::cout << this->_name << ": ";
