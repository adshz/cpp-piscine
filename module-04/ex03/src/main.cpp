...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/24 11:01:57 by szhong            #+#    #+#             */
/*   Updated: 2025/07/24 11:01:58 by szhong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void testMateriaSource(void) {
  ...
  std::cout << std::endl
            << "-- MATERIA SOURCE LEARNING TOO MANY RECIPES" << std::endl;
  source->displayKnownRecipes();
  source->learnMateria(new Ice());
  source->learnMateria(new Cure());
  source->displayKnownRecipes();
  ...
  std::cout << std::endl
            << "-- MATERIA SOURCE CREATING MATERIA FROM RECIPES" << std::endl;
  AMateria *m1 = NULL;
  AMateria *m2 = NULL;
  AMateria *m3 = NULL;
...
void testCharacters(void) {
  ...
  std::cout << std::endl
            << "-- CHARACTER EQUIPPING TOO MANY ITEMS" << std::endl;
  emily->displayInventory();
  emily->equip(m4);
  emily->equip(m5);
  emily->equip(m6);
  ...
  std::cout << std::endl
            << "-- CHARACTER UNEQUIPPING ITEM DOES NOT DELETE IT" << std::endl;
...
void testMateriaDuelBetweenTwoCharacters(void) {
  ...
  std::cout << "-- Creating an array of " << nb_items << " materias."
            << std::endl;
