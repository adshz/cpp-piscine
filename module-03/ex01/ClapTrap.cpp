...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/23 15:54:41 by szhong            #+#    #+#             */
/*   Updated: 2025/07/23 15:54:44 by szhong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap(void)
    : _name("Default"), _hitPoints(CLAPTRAP_DEFAULT_HIT_POINTS),
...
ClapTrap::ClapTrap(std::string name)
    : _name(name), _hitPoints(CLAPTRAP_DEFAULT_HIT_POINTS),
      _energyPoints(CLAPTRAP_DEFAULT_ENERGY_POINTS),
      _attackDamage(CLAPTRAP_DEFAULT_ATTACK_DAMAGE),
      _maxHitPoints(CLAPTRAP_DEFAULT_HIT_POINTS),
      _maxEnergyPoints(CLAPTRAP_DEFAULT_ENERGY_POINTS),
      _maxAttackDamage(CLAPTRAP_DEFAULT_ATTACK_DAMAGE) {
  std::cerr << CYAN "A ClapTrap named \"" << name << "\" was constructed." RESET
            << std::endl;
  return;
}
...
ClapTrap::~ClapTrap(void) {
  std::cerr << CYAN "The ClapTrap named \"" << this->_name
...
void ClapTrap::takeDamage(unsigned int amount) {
  ...
  if ((int)this->_hitPoints - (int)amount <= 0) {
    this->_hitPoints = 0;
    std::cout << RED << this->_name << " takes " << amount
              << " damage and is destroyed!" RESET << std::endl;
  } else {
    this->_hitPoints -= amount;
    std::cout << RED << this->_name << " takes " << amount << " damage!" RESET
              << std::endl;
  }
  return;
}
...
void ClapTrap::beRepaired(unsigned int amount) {
  ...
  if (amount == 0) {
    this->_energyPoints--;
    std::cout << GREEN << this->_name
              << " wastes energy trying to repair itself when it is already in "
                 "tip top shape." RESET
              << std::endl;
    return;
  }
  this->_energyPoints--;
  this->_hitPoints += amount;
  std::cout << GREEN << this->_name << " repairs itself for " << amount
            << "." RESET << std::endl;
  return;
}
